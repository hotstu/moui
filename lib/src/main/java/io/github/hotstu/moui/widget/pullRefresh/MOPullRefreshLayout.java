 ...
 * <li>可在 xml 中使用 {@link io.github.hotstu.moui.R.styleable#MOPullRefreshLayout} 这些属性或在 Java 设置对应的属性决定子View的开始位置、触发刷新的位置等值</li>
 * </ul>
 *
 * @author cginechen
 * @date 2016-12-11
 */
public class MOPullRefreshLayout extends ViewGroup implements NestedScrollingParent {
    ...
    public MOPullRefreshLayout(Context context) {
        this(context, null);
    }

    public MOPullRefreshLayout(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.MOPullRefreshLayoutStyle);
    }

    public MOPullRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTouchSlop = MODisplayHelper.px2dp(context, mSystemTouchSlop); //系统的值是8dp,如何配置？
        ...
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.MOPullRefreshLayout,
                defStyleAttr, R.style.MOPullRefreshLayout);

        try {
            mRefreshInitOffset = array.getDimensionPixelSize(
                    R.styleable.MOPullRefreshLayout_mo_refresh_init_offset, Integer.MIN_VALUE);
            mRefreshEndOffset = array.getDimensionPixelSize(
                    R.styleable.MOPullRefreshLayout_mo_refresh_end_offset, Integer.MIN_VALUE);
            mTargetInitOffset = array.getDimensionPixelSize(
                    R.styleable.MOPullRefreshLayout_mo_target_init_offset, 0);
            mTargetRefreshOffset = array.getDimensionPixelSize(
                    R.styleable.MOPullRefreshLayout_mo_target_refresh_offset,
                    MODisplayHelper.dp2px(getContext(), 72));
            mAutoCalculateRefreshInitOffset = mRefreshInitOffset == Integer.MIN_VALUE ||
                    array.getBoolean(R.styleable.MOPullRefreshLayout_mo_auto_calculate_refresh_init_offset, false);
            mAutoCalculateRefreshEndOffset = mRefreshEndOffset == Integer.MIN_VALUE ||
                    array.getBoolean(R.styleable.MOPullRefreshLayout_mo_auto_calculate_refresh_end_offset, false);
            mEqualTargetRefreshOffsetToRefreshViewHeight = array.getBoolean(R.styleable.MOPullRefreshLayout_mo_equal_target_refresh_offset_to_refresh_view_height, false);
        } finally {
            array.recycle();
        }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (!isEnabled() || canChildScrollUp() || mNestedScrollInProgress) {
            //Log.d(TAG, "fast end onIntercept: isEnabled = " + isEnabled() + "; canChildScrollUp = " + canChildScrollUp() + " ; mNestedScrollInProgress = " + mNestedScrollInProgress);
            return false;
        }
        switch (action) {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (!isEnabled() || canChildScrollUp() || mNestedScrollInProgress) {
            //Log.d(TAG, "fast end onTouchEvent: isEnabled = " + isEnabled() + "; canChildScrollUp = " + canChildScrollUp() + " ; mNestedScrollInProgress = " + mNestedScrollInProgress);
            return false;
        }
    ...
    private void finishPull(int vy) {
        //Log.i(TAG, "finishPull: vy = " + vy + " ; mTargetCurrentOffset = " + mTargetCurrentOffset + " ; mTargetRefreshOffset = " + mTargetRefreshOffset + " ; mTargetInitOffset = " + mTargetInitOffset + " ; mScroller.isFinished() = " + mScroller.isFinished());
    ...
    public boolean onNestedPreFling(View target, float velocityX, float velocityY) {
        //Log.i(TAG, "onNestedPreFling: mTargetCurrentOffset = " + mTargetCurrentOffset + " ; velocityX = " + velocityX + " ; velocityY = " + velocityY);
    ...
    public interface OnChildScrollUpCallback {
        boolean canChildScrollUp(MOPullRefreshLayout parent, @Nullable View child);
    }
    ...
    public static class RefreshView extends AppCompatImageView implements IRefreshView {
        ...
        public RefreshView(Context context) {
            ...
            mProgress.setColorSchemeColors(MOResHelper.getAttrColor(context, R.attr.mo_colorPrimary));
